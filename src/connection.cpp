...
namespace raxvt
{
  ...
  void
  connection::send(const char *data, std::size_t length)
  {
    uint8_t s[2];
  ...
  void
  connection::send(const char* data)
  {
    send(data, std::strlen(data));
  }

  bool
  connection::recv(std::string& data, std::size_t* length)
  {
    uint8_t s[2];
    std::size_t l;
    char* buffer;

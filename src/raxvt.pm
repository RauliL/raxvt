...
raxvtperl - raxvt's embedded perl interpreter
...
=head1 SYNOPSIS
   ...
   # start a raxvt using it:

   raxvt --perl-lib $HOME -pe grab_test
...
Each script will only ever be loaded once, even in raxvtd, where
...
in F<< <libdir>/raxvt/perl/ >>, and the documentation can be viewed using
F<< man raxvt-<EXTENSIONNAME> >>.

You can activate them like this:

  raxvt -pe <extensionname>

Or by adding them to the resource for extensions loaded by default:

  raxvt.perl-ext-common: default,selection-autotransform
...
Although it isn't a C<raxvt::term> object, you can call all methods of the
C<raxvt::term> class on this object.

Additional methods only supported for extension objects are described in
the C<raxvt::extension> section below.
...
Returning a true value aborts selection making by raxvt, in which case you
...
description of the B<keysym> resource in the raxvt(1) manpage). The
...
resource in the raxvt(1) manpage).
...
=head2 Variables in the C<raxvt> Package
...
=item $raxvt::RESCLASS, $raxvt::RESCLASS

The resource class and name rxvt-unicode uses to look up X resources.

=item $raxvt::RXVTNAME

The basename of the installed binaries, usually C<raxvt>.

=item $raxvt::TERM

The current terminal. This variable stores the current C<raxvt::term>
object, whenever a callback/hook is executing.

=item @raxvt::TERM_INIT

All code references in this array will be called as methods of the next newly
created C<raxvt::term> object (during the C<on_init> phase). The array
...
=head2 Functions in the C<raxvt> Package

=over 4

=item raxvt::fatal $errormessage
...
correct place, e.g. on stderr of the connecting raxvtc client.

Messages have a size limit of 1023 bytes currently.

=item @terms = raxvt::termlist

Returns all raxvt::term objects that exist in this process, regardless of
...
is no raxvt::term object associated with others).

=item $time = raxvt::NOW

Returns the "current time" (as per the event loop).

=item raxvt::CurrentTime

=item raxvt::ShiftMask, LockMask, ControlMask, Mod1Mask, Mod2Mask,
Mod3Mask, Mod4Mask, Mod5Mask, Button1Mask, Button2Mask, Button3Mask,
Button4Mask, Button5Mask, AnyModifier

=item raxvt::NoEventMask, KeyPressMask, KeyReleaseMask,
...
=item raxvt::KeyPress, KeyRelease, ButtonPress, ButtonRelease, MotionNotify,
...
=item raxvt::PrivMode_132, PrivMode_132OK, PrivMode_rVideo, PrivMode_relOrigin,
...
=item $rend = raxvt::DEFAULT_RSTYLE
...
=item $rend = raxvt::OVERLAY_RSTYLE

Return the rendition mask used for overlays by default.

=item $rendbit = raxvt::RS_Bold, raxvt::RS_Italic, raxvt::RS_Blink,
raxvt::RS_RVid, raxvt::RS_Uline

Return the bit that enabled bold, italic, blink, reverse-video and
underline, respectively. To enable such a style, just logically OR it into
the bitset.

=item $foreground = raxvt::GET_BASEFG $rend

=item $background = raxvt::GET_BASEBG $rend

Return the foreground/background colour index, respectively.

=item $rend = raxvt::SET_FGCOLOR $rend, $new_colour

=item $rend = raxvt::SET_BGCOLOR $rend, $new_colour

=item $rend = raxvt::SET_COLOR $rend, $new_fg, $new_bg

Replace the foreground/background colour in the rendition mask with the
specified one.

=item $value = raxvt::GET_CUSTOM $rend

Return the "custom" value: Every rendition has 5 bits for use by
extensions. They can be set and changed as you like and are initially
zero.

=item $rend = raxvt::SET_CUSTOM $rend, $new_value
...
sub usage {
   ...
   for my $pattern (sort keys %$r) {
      ...
      if ($usage_type == 1) {
         ...
         if ($type eq "boolean") {
            raxvt::log sprintf "  -%-30s %s\n", "/+$pattern", $desc;
         } else {
            raxvt::log sprintf "  -%-30s %s\n", "$pattern $type", $desc;
         }
      } else {
         $pattern =~ s/\.$/.*/g;
         raxvt::log sprintf "  %-31s %s\n", "$pattern:", $type;
      }
   }
}

my $verbosity = $ENV{raxvt_PERL_VERBOSITY};
...
our $retval; # return value for raxvt
...
package raxvt::term::extension;

=head2 The C<raxvt::term::extension> class

Each extension attached to a terminal object is represented by
a C<raxvt::term::extension> object.
...
objects, in addition to call methods documented for the <raxvt::term>
class.

=over 4

=item $raxvt_term = $self->{term}

Returns the C<raxvt::term> object associated with this instance of the
...
# raxvt::destroy_hook (basically a cheap Guard:: implementation)

sub raxvt::destroy_hook::DESTROY {
   ${$_[0]}->();
}

sub raxvt::destroy_hook(&) {
   bless \shift, raxvt::destroy_hook::
}
...
method of the C<raxvt::term> class.
...
sub raxvt::extension::on_disable::DESTROY {
   my $disable = shift;
...
sub on {
   ...
   bless \%disable, "raxvt::extension::on_disable"
}
...
C<raxvt::term> methods of the same name - see the description of these
methods in the C<raxvt::term> class for details.
...
package raxvt::anyevent;

=head2 The C<raxvt::anyevent> Class

The sole purpose of this class is to deliver an interface to the
C<AnyEvent> module - any module using it will work inside raxvt without
...
$INC{"raxvt/anyevent.pm"} = 1; # mark us as there
push @AnyEvent::REGISTRY, [raxvt => raxvt::anyevent::];
...
sub one_event {
   Carp::croak "AnyEvent->one_event blocking wait unsupported in raxvt, use a non-blocking API";
}

package raxvt::term;

=head2 The C<raxvt::term> Class
...
sub register_package {
   ...
   raxvt::verbose 6, "register package $pkg to $self";

   @{"$pkg\::ISA"} = raxvt::term::extension::;
...
sub scan_extensions {
   ...
   my @raxvtdirs = perl_libdirs $self;
...
=item $term = new raxvt::term $envhashref, $rxvtname, [arg...]
...
sub new {
   ...
   $env  or Carp::croak "environment hash missing in call to raxvt::term->new";
   @args or Carp::croak "name argument missing in call to raxvt::term->new";
...
etc.). Please note that raxvt will not exit as long as any event
...
C<%raxvt::OPTION>. Options not enabled in this binary are not in the hash.
...
C<keysym> resource in the raxvt(1) manpage.
...
This method returns an raxvt::overlay object. The overlay will be visible
as long as the perl object is referenced.

The methods currently supported on C<raxvt::overlay> objects are:
...
Creates a new C<raxvt::popup> object that implements a popup menu. The
...
sub popup {
   ...
   $self->{_destroy}{$popup} = raxvt::destroy_hook { $popup->{popup}->destroy };
...
description of C<< raxvt::timer->events >>. Make sure to always restore
the previous value.
...
to receive pointer events all the times:
   ...
   $term->vt_emask_add (raxvt::PointerMotionMask);
...
Example:
  ...
  if ($term->priv_modes & raxvt::PrivMode_mouse_report) {
...
than one cell when displayed are padded with C<$raxvt::NOCHAR> (chr 65535)
...
length"). Unlike the raxvt core, this returns C<< $term->ncol >> if the
...
Create and return a new C<raxvt::line> object that stores information
...
sub raxvt::line::beg { $_[0]{beg} }
sub raxvt::line::end { $_[0]{end} }
sub raxvt::line::l   { $_[0]{len} }

sub raxvt::line::offset_of {
   my ($self, $row, $col) = @_;

   ($row - $self->{beg}) * $self->{ncol} + $col
}

sub raxvt::line::coord_of {
   my ($self, $offset) = @_;
...
sub show {
   ...
   unless (delete $term->{raxvt_popup_init_done}) {
      $term->ungrab;
      $term->destroy;
...
package raxvt::watcher;

=head2 The C<raxvt::timer> Class

This class implements timer watchers/events. Time is represented as a
fractional number of seconds since the epoch. Example:

   $term->{overlay} = $term->overlay (-1, 0, 8, 1, raxvt::OVERLAY_RSTYLE, 0);
   $term->{timer} = raxvt::timer
                    ->new
                    ->interval (1)
                    ->cb (sub {
                       $term->{overlay}->set (0, 0,
                          sprintf "%2d:%02d:%02d", (localtime raxvt::NOW)[2,1,0]);
                    });

=over 4

=item $timer = new raxvt::timer
...
Like C<start>, but sets the expiry timer to c<raxvt::NOW + $delay>.
...
C<raxvt::EV_READ> and C<raxvt::EV_WRITE>, which might be ORed
together, or C<raxvt::EV_NONE>.
